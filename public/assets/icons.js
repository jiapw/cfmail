...
const P = {
  ...
  stamp: 'M12 2a4 4 0 0 0-4 4c0 1.86 1.28 3.41 3 3.86V12H7a3 3 0 0 0-3 3v2h16v-2a3 3 0 0 0-3-3h-4v-2.14c1.72-.45 3-2 3-3.86a4 4 0 0 0-4-4zM4 19h16v2H4v-2z',
  signature: 'M18.85 10.39l1.06-1.06c.78-.78.78-2.05 0-2.83L18.5 5.09c-.78-.78-2.05-.78-2.83 0l-1.06 1.06 4.24 4.24zm-5.66-2.83L4 16.76V21h4.24l9.19-9.19-4.24-4.25zM19 17.5c0 2.19-2.54 3.5-5 3.5-.55 0-1-.45-1-1s.45-1 1-1c1.54 0 3-.73 3-1.5 0-.47-.48-.87-1.23-1.2l1.48-1.48c1.07.63 1.75 1.47 1.75 2.68z',
  ...
  // Light and dark, for the fill page's mode toggle -- a visitor with no account has no settings page to do it from
  // 明与暗,给填写页的明暗切换用 —— 没有账号的访问者没有设置页可去切
  ...
  // A disc with the letter knocked out of it, the way star knocks out its own middle:
  // the two sub-paths run in opposite directions, so the fill cancels where they overlap.
  // 一个把字母抠掉的圆盘,做法与 star 抠掉自己中间那块相同:
  // 两条子路径方向相反,于是重叠处的填充相互抵消。
  info: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z',
  check: 'M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z',
  // The player's own controls. Nothing here comes from the browser any more, so the pictures on
  // the buttons do not either.
  // 播放器自己的那套控件。这里再没有一样东西来自浏览器,所以按钮上的图形也不来自浏览器。
  play: 'M8 5v14l11-7z',
  ...
  windowed: 'M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z',
  subtitles:
    'M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zM4 12h4v2H4v-2zm10 6H4v-2h10v2zm6 0h-4v-2h4v2zm0-4H10v-2h10v2z',
  // Nothing, drawn: it holds the space a tick would take, so a menu where one row is ticked
  // does not have every other row shifted left.
  // 什么都不画,但占住一个对勾的位置 —— 这样一份"有一行打了勾"的菜单,
  // 不会让其余每一行都往左挪。
  blank: 'M0 0h0v0z',
