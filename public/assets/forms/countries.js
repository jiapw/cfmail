// ISO 3166-1 alpha-2 codes with English short names. The server holds the same list in
// src/countries.ts (the mail is composed there); this copy feeds the country picker, which shows
// each name in the reader's own language through Intl.DisplayNames and falls back to English.
// ISO 3166-1 两位代码及英文短名。服务端在 src/countries.ts 持有同一份(邮件在那里组成);
// 这一份供国家选择器使用,经 Intl.DisplayNames 以读者自己的语言显示,取不到时回落英文。
...
export const COUNTRIES = [
  ...
  ['KZ', 'Kazakhstan'], ['KE', 'Kenya'], ['KI', 'Kiribati'], ['KP', "Korea (Democratic People's Republic)"], ['KR', 'Korea (Republic)'],
...
/** The country's name as the reader would call it, with the English name as the fallback.
 *  读者会怎么称呼这个国家;取不到就用英文名。 */
export function countryName(code, lang) {
  try {
    const dn = new Intl.DisplayNames([lang || navigator.language], { type: 'region' });
    const n = dn.of(code);
...
/** All countries sorted by their name in the given language.
 *  全部国家,按给定语言的名字排序。 */
export function countryOptions(lang) {
  const list = COUNTRIES.map(([code]) => ({ code, name: countryName(code, lang) }));
  try { list.sort((a, b) => a.name.localeCompare(b.name, lang || undefined)); } catch { list.sort((a, b) => (a.name < b.name ? -1 : 1)); }
  return list;
}
