...
//
// So nothing is queued. One job runs at a time, and whenever the worker is free it looks at
// where the viewport is NOW and takes the nearest page that has not been built. A page that was
// interesting a moment ago and is no longer on screen is simply never chosen. The effect is
...
// 他真正在看的那一页排在所有人后面。
//
// 所以这里不排队。同一时刻只跑一个任务,worker 每次空闲都去看视口"此刻"在哪,
 ...
 */
export function lazyPages({ root, items, render, margin = 600 }) {
  const done = new Set();
  let busy = false;
  let dead = false;
  ...
  const pump = async () => {
    ...
    busy = true;
    try {
      await render(next.el, next.i);
    } catch { /* one page failing must not stop the rest / 一页失败不该拖住其余 */ } finally {
      // Marked done either way: a page that throws would otherwise be chosen again forever
      // 无论成败都记为已完成:否则一个抛错的页面会被永远重新选中
      done.add(next.el);
      busy = false;
      if (!dead) pump();
    }
