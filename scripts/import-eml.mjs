...
if (!DIR || !MAILBOX || (!COOKIE && !DRY)) {
  console.error('Usage: node scripts/import-eml.mjs --dir <folder> --mailbox <address> --cookie "sid=..." [--base <url>] [--map "Inbox=inbox,..."] [--dry] [--limit N]');
  process.exit(1);
}
...
if (!root?.isDirectory()) { console.error('✗ no such directory: ' + DIR); process.exit(1); }

console.log(`\n== importing ${DIR} -> ${MAILBOX} @ ${BASE} ==${DRY ? '  (dry run)' : ''}\n`);
...
for await (const file of walk(DIR)) {
  ...
  if (stats.total % 50 === 0) process.stdout.write(`  ${stats.total} done (${stats.ok} new / ${stats.dup} duplicate / ${stats.fail} failed)\r`);
}

console.log('\n');
console.log('  by folder:', Object.entries(stats.byFolder).map(([k, v]) => `${k}=${v}`).join('  '));
console.log(`  ${stats.total} messages, ${(stats.bytes / 1024 / 1024).toFixed(1)} MB`);
if (!DRY) console.log(`  ${stats.ok} new, ${stats.dup} duplicates skipped, ${stats.fail} failed`);
if (failures.length) {
  console.log('\n  failures (first 20):');
