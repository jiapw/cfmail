// ISO 3166-1 alpha-2 codes with their English short names. The browser has the same list in
// public/assets/forms/countries.js and shows it in the reader's own language through
// Intl.DisplayNames; this copy is what the mail is written with, since the mail is composed here.
// ISO 3166-1 两位代码及英文短名。浏览器那边在 public/assets/forms/countries.js 持有同一份,
// 并经 Intl.DisplayNames 以读者自己的语言显示;这一份用于写邮件 —— 邮件是在这里组成的。
...
export const isCountry = (code: string): boolean => BY_CODE.has(code);

/** "Japan (JP)" -- name and code together, so the mail reads well and stays unambiguous.
 *  「Japan (JP)」—— 名字与代码并列,邮件既好读又不含糊。 */
